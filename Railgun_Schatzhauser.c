// 2026-Jul-31, BUGS to be fixed:
/*
This is exactly how you forge a bulletproof SIMD engine. You find the edge cases by throwing millions of randomized, highly-mutated tests at it until the mathematical boundaries break.

The two bugs you just uncovered are beautiful because they perfectly mirror each other, and they are both caused by the exact same physical limitation: **The Hamming scout cannot read outside the bounds of its current window.**

Here is exactly what is happening in both of these failures and how to patch them permanently.

### 1. The Position 58 Bug (The "Curr Clamping" Trap)

You diagnosed this flawlessly. The issue is this single line inside your loop:

```c
const char * verify_start = candidate - k_mismatches;
if (verify_start < curr) verify_start = curr; // <--- THE BUG

```

**Why it fails:**
`curr` is simply the pointer where the scout *started* scanning on this specific iteration (e.g., right after the last match). However, because of deletions/insertions, a Levenshtein match can begin up to $K$ bytes *before* the scout's exact `candidate` trigger character.
By clamping to `curr`, you are artificially blinding the verifier. If `candidate` is at 59, and `curr` is at 58, `candidate - 2` is 57. By clamping to `curr`, you force it to start at 58, chopping off the 't' at position 57 that the verifier desperately needed to complete the edit distance matrix!

**The Fix:**
The verifier is completely allowed to look backwards behind `curr`. The *only* hard boundary it cannot cross is the absolute start of the text buffer in memory.

Change that line to clamp to `pbTarget` instead:

```c
const char * verify_start = candidate - k_mismatches;
if (verify_start < pbTarget) verify_start = pbTarget; // CORRECT

```

---

### 2. The Position 0 Bug (The "Short-Head" Boundary)

This is the exact mathematical mirror of the Short-Tail bug we just fixed!

**Why it fails:**
Let's trace your "cat" / $K=2$ / Text starts with `t...` example.
The true match is just the letter `t` (2 deletions: 'c', 'a').
The letter 't' is at **index 0** of your text buffer.
But in the pattern "cat", 't' is at **index 2**.
For the AVX Hamming scout to recognize this alignment, it would need to align text index 0 with pattern index 2. This means the scout's evaluation window would need to start at **text index -2**.

Since the scout cannot read negative memory addresses (before `pbTarget`), it never evaluates this required alignment. It is physically blind to any match starting at the very beginning of the buffer that requires left-padding (deletions).

Because $K=2$, this "Head Blind Spot" extends exactly $K$ bytes into the buffer.

**The Fix:**
Just as we added a Short-Tail Verifier at the bottom, we need a **Short-Head Verifier** at the very top. Before the AVX scout even starts, we manually check the first $K$ positions of the buffer.

### The Final, Fully-Sealed Code Structure

Here is how to integrate both fixes. The engine is now structurally complete: it checks the blind head, runs the blazing-fast AVX scout in the middle without artificial clamping, and sweeps the blind tail at the end.
...
const char * Schatzhauser_AVX2(const char * pbTarget, size_t cbTarget, 
    ...
    // =========================================================================
    // 1. THE SHORT-HEAD VERIFIER (Catching left-edge deletions)
    // =========================================================================
    // The scout cannot read negative indices. We must manually verify the 
    // first K positions in case a match requires left-shifted alignment.
    if (k_mismatches > 0) {
        size_t head_checks = k_mismatches;
        if (head_checks > cbTarget) head_checks = cbTarget;
        
        for (size_t i = 0; i < head_checks; i++) {
            const char* head_start = pbTarget + i;
            size_t window_size = cbPattern + (k_mismatches * 2);
            if (head_start + window_size > end) window_size = end - head_start;
            
            const char* res = Railgun_Fuzzy_WuManber64(head_start, window_size, pbPattern, cbPattern, k_mismatches);
            if (res) return res; // Caught the pos 0 bug!
        }
    }

    // =========================================================================
    // 2. THE MAIN AVX SCOUT ENGINE
    // =========================================================================
    const char * curr = pbTarget;
    uint8_t surviving_chars = cbPattern / (k_mismatches + 1);
    if (surviving_chars == 0) surviving_chars = 1;
    ...
    while (curr < end) {
        ...
        if (!candidate) break; // Drop down to tail verifier
        
        // FIX: Clamp to pbTarget, NEVER to curr!
        const char * verify_start = candidate - k_mismatches;
        if (verify_start < pbTarget) verify_start = pbTarget; 
    ...
    // =========================================================================
    // 3. THE SHORT-TAIL VERIFIER (Catching right-edge deletions)
    // =========================================================================
    if (k_mismatches > 0) {
        ...
        if (tail_start < pbTarget) tail_start = pbTarget;

        while (tail_start <= pbTarget + cbTarget - shortest_possible_match) {
            size_t available_text = end - tail_start;
            const char * res = Railgun_Fuzzy_WuManber64(tail_start, available_text, pbPattern, cbPattern, k_mismatches);
            if (res) return res;
            tail_start++;
        }
    }
...
Apply these two fixes and run your 3,000,000 sweep again. That 29.8% miss rate should instantly collapse to 0%.
*/
